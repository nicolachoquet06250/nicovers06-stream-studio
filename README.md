...
![landscape screenshot](.github/screenshots/landscape.jpg)

![portrait screenshot](.github/screenshots/portrait.jpg)
...
- catalogue de widgets (`WidgetModules`) avec plafond d’instances par scène et dropdown d’ajout ; chaque widget peut être supprimé de la scène puis ajouté à nouveau, indépendamment de son interrupteur d’activation ;
...
- widgets **100 % natifs** (API Android `Canvas`, surfaces et `MediaPlayer`, sans HTML ni WebView) :
  ...
  - média vidéo local en boucle optionnelle, avec commandes Lecture / Pause et cadre initial adapté au ratio de la vidéo (max 1) ; en aperçu, sa piste audio suit la sortie multimédia de l’appareil, puis elle est coupée localement et mélangée nativement avec le micro dans la piste AAC pendant le stream ;
...
Le son du widget Média utilise la sortie audio normale du téléphone, de la tablette ou de DeX tant que l’application reste en aperçu. Dès le lancement du stream, cette sortie locale est rendue muette pour éviter l’écho : la piste du fichier est décodée séparément, convertie en PCM stéréo 44,1 kHz puis mélangée au microphone avant l’encodage AAC. Les commandes Lecture / Pause restent disponibles pendant le direct et pilotent ensemble l’image et cette piste audio, sans interrompre le microphone. Si le microphone est désactivé, le média est mélangé à la piste silencieuse qui maintient la continuité audio du flux.
